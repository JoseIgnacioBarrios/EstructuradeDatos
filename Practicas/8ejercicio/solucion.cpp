...
int recursiva(BinTree<string> arbol, MapTree<string,int> expresion) {
    int valorClave=0;
    //if (arbol.empty())return 0;//caso base arbol vacio
    ...
    else
    {
        ...
        izq = recursiva(arbol.left(), expresion);
        der = recursiva(arbol.right(), expresion);
        ...
        else if (expresion.contains(arbol.root()))
        {
            valorClave = expresion.at(arbol.root());
        }
        else
        {
            valorClave = stoi(arbol.root());
        }
...
bool tratar_caso() {
    ...
    //string =clave
    //int = valor
    MapTree<string, int> expresion;
    for (int i = 0; i < n; i++) {
        //crear unn arbol par al inicializacion
        BinTree<string> arbol = parse(cin);
        //asignar la primera asignacion X=
        string variable = arbol.left().root();
        int formula = recursiva(arbol.right(), expresion);
        expresion[{variable}] = formula;
    }
    for (auto var : expresion)
    {
        cout << var.key << " = " << var.value << "\n";
    }
    cout << "---" << "\n";
}
