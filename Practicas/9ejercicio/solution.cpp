 ...
 * ====================================================================== */
/*@ <answer> */
int recorrido(BinTree<string> tree, MapTree<string,int> &expresion) {
    if (tree.empty()) {
        return 0;
    }
    else {
        int iz = recorrido(tree.left(), expresion);
        int dr = recorrido(tree.right(), expresion);
        int res = 0;
        if (tree.root() == "+") {
            res = iz + dr;
        }
            ...
            //quitar el contains por isdigit
            if (expresion.contains(tree.root())) {
                res = expresion.at(tree.root());
            }
            else {
                stringstream(tree.root()) >> res;
            }
        }
...
bool tratar_caso() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        int formula = recorrido(arbol.right(), expresion);
        expresion[{variable}] = formula;
    }

    /*Salida a pantalla*/
    for (auto& [key, value] : expresion) {
        cout << key << " = " << value << endl;
    }
    cout << "---" << endl;
    return true;
